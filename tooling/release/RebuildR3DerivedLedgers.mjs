...
'PCA-FR-130': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  sourceEvidence: ['backend/src/childrequests/ChildRequestService.ts', 'backend/src/childrequests/BonusGrantLedger.ts', 'backend/src/childrequests/policy.ts', 'backend/src/childrequests/types.ts', 'backend/src/childrequests/ChildRequestRepository.ts', 'backend/src/http/routes/childRequestRoutes.ts', 'backend/src/http/buildServer.ts', 'backend/src/familyrbac/policy.ts', 'android/app/src/main/java/org/pca/app/runtime/schedule/BonusGrantSync.kt', 'android/app/src/main/java/org/pca/app/runtime/ui/ChildHomeScreen.kt', 'parent-web/src/pages/Requests.tsx', 'parent-web/src/domain/bonusTime.ts', 'parent-web/src/api/real/realRequestClient.ts', 'parent-web/src/api/dev/devRequestClient.ts'],
  testEvidence: ['backend/test/childrequests/ChildRequestService.test.mjs', 'backend/test/childrequests/BonusGrantLedger.test.mjs', 'backend/test/http/childRequestRoutes.test.mjs', 'backend/test/schedule/engine.test.mjs', 'android/app/src/test/java/org/pca/app/runtime/schedule/BonusGrantSyncTest.kt', 'parent-web/tests/component/Requests.test.tsx'],
  sourceSolvableClass: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  externalGate: ['PRODUCTION_CRYPTO_SUITE'],
  currentGap: 'P23-Writer01 (2026-08-22, commit fe0e61f) closed this under standing decision PARENT_AUTHORIZED_BOUNDED_BONUS_TIME: a real, RBAC-gated (Owner/Administrator ALLOW, Viewer DENY, Child REQUEST_ONLY), audited, idempotent, bounded (120 min) bonus-time request/approve/deny/counter-offer flow, reusing ParentActionAuthorizationService, ChildProfileMembershipResolver, FamilyAuditService, and the existing BonusGrant/evaluateSchedule enforcement primitive (bedtime/emergency precedence and absolute-UTC expiry apply for free). Independently re-verified by P23-Writer07 (2026-08-22) against real source and a real test re-run (backend 1679/1679, Android BUILD SUCCESSFUL, parent-web 476/476); P23-Writer05 red-team and P23-Writer06 mutation testing (8 targeted mutants) found zero survivors. Remaining gap is pre-existing and shared with every other real-transport path in this codebase: production decide()/grantDirectly() fails closed pending the PRODUCTION_CRYPTO_SUITE human security review, and no live E2EE family-sync transport exists yet on either platform to carry a decision to the child device.',
  nextAction: 'Bind the live family-sync transport and clear PRODUCTION_CRYPTO_SUITE; no further source-level bonus-time work required.',
  notes: 'P23 combined Prompt-2/3 programme (2026-08-22): closed under standing decision D1 (PARENT_AUTHORIZED_BOUNDED_BONUS_TIME); reclassified from OWNER_DECISION_REQUIRED_FOR_SOURCE to SOURCE_COMPLETE_EXTERNAL_GATE.',
},
'PCA-FR-131': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  sourceEvidence: ['backend/src/childrequests/types.ts', 'backend/src/childrequests/ChildRequestService.ts', 'backend/src/childrequests/policy.ts', 'backend/src/familyrbac/policy.ts', 'backend/src/familyrbac/types.ts', 'backend/src/http/routes/childRequestRoutes.ts', 'android/app/src/main/java/org/pca/app/feature/installapproval/InstallApprovalCapabilityState.kt', 'android/app/src/main/java/org/pca/app/feature/installapproval/InstallApprovalController.kt', 'android/app/src/main/java/org/pca/app/feature/installapproval/InstallApprovalDecisionApplier.kt', 'android/app/src/main/java/org/pca/app/runtime/installobserver/InstalledAppEventReceiver.kt', 'ios/PCA/InstallApproval/InstallApprovalCapability.swift', 'parent-web/src/pages/Requests.tsx', 'parent-web/src/domain/types.ts'],
  testEvidence: ['backend/test/childrequests/ChildRequestService.test.mjs', 'backend/test/http/childRequestRoutes.test.mjs', 'backend/test/familyrbac/policy.test.mjs', 'android/app/src/test/java/org/pca/app/feature/installapproval/InstallApprovalCapabilityResolverTest.kt', 'android/app/src/test/java/org/pca/app/feature/installapproval/InstallApprovalControllerTest.kt', 'android/app/src/test/java/org/pca/app/feature/installapproval/InstallApprovalDecisionApplierTest.kt', 'ios/PCATests/InstallApprovalCapabilityTests.swift', 'parent-web/tests/component/Requests.test.tsx'],
  sourceSolvableClass: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  externalGate: ['PRODUCTION_CRYPTO_SUITE', 'IOS_MAC_XCODE'],
  currentGap: 'P23-Writer02 (2026-08-22, commit ce32e9c) closed this under standing decision CAPABILITY_HONEST_INSTALL_APPROVAL: enforcement is claimed only where OS authority is genuinely proven. installCapabilityState (request-time snapshot) and installEnforcementOutcome (device post-decision report) are deliberately separate, never-merged fields, so approved can never be conflated with enforced. Android quarantines only when live capability resolves ENFORCED, re-resolving at apply time so authority loss degrades honestly; iOS reports a categorical, permanent NOT_SUPPORTED verified against the real public FamilyControls/ManagedSettings/DeviceActivity surface (no install-observation callback exists) -- a genuine platform ceiling, not a gap. Independently re-verified by P23-Writer07 (2026-08-22): backend 1679/1679, Android BUILD SUCCESSFUL, parent-web 476/476; iOS test file exists but could not be executed (no macOS/Xcode toolchain). P23-Writer05 red-team and P23-Writer06 mutation testing found zero survivors on the approved/enforced-conflation and authority-loss-race attack paths. P23-Writer12 (commit d95b5d8) found and fixed a real project.pbxproj target-membership gap: InstallApprovalCapability.swift and its test had been added to disk by ce32e9c but never wired into the Xcode Sources build phase; both are now correctly wired and mechanically verified (no duplicate object IDs, correct group/target membership) without Xcode. Remaining gaps: PRODUCTION_CRYPTO_SUITE (decide() fails closed; no live family-sync transport on either platform) and IOS_MAC_XCODE, now narrowed to pure external validation (source is target-membership-complete; only compilation/build/device execution on a real Mac remains outstanding).',
  nextAction: 'Build on macOS/Xcode (source is already wired into project.pbxproj); bind the live family-sync transport and clear PRODUCTION_CRYPTO_SUITE.',
  notes: 'P23 combined Prompt-2/3 programme (2026-08-22): closed under standing decision D2 (CAPABILITY_HONEST_INSTALL_APPROVAL); reclassified from OWNER_DECISION_REQUIRED_FOR_SOURCE to SOURCE_COMPLETE_EXTERNAL_GATE. P23-Writer12 (commit d95b5d8) found and fixed a real project.pbxproj target-membership gap for InstallApprovalCapability.swift and its test (added to disk by ce32e9c but never wired into the Xcode Sources build phase); both are now correctly wired and mechanically verified without Xcode. IOS_MAC_XCODE is now narrowed to pure external validation (compile/build/device execution on a real Mac); no source defect remains.',
},
'PCA-NFR-034': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  sourceEvidence: ['android/app/src/main/java/org/pca/app/runtime/background/PcaBatteryBudgetPolicy.kt', 'android/app/src/main/java/org/pca/app/runtime/background/BatteryBudgetedCycleRunner.kt', 'android/app/src/main/java/org/pca/app/runtime/background/BackgroundExecutionScheduler.kt', 'android/app/src/main/java/org/pca/app/platform/PowerSaveModeSource.kt', 'android/app/src/main/java/org/pca/app/runtime/graph/PcaAppGraph.kt'],
  testEvidence: ['android/app/src/test/java/org/pca/app/runtime/background/PcaBatteryBudgetPolicyTest.kt', 'android/app/src/test/java/org/pca/app/runtime/background/BatteryBudgetedCycleRunnerTest.kt', 'android/app/src/test/java/org/pca/app/platform/StandardPowerSaveModeSourceTest.kt', 'android/app/src/test/java/org/pca/app/runtime/graph/PcaAppGraphTest.kt'],
  sourceSolvableClass: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  externalGate: ['ANDROID_REAL_DEVICE_UAT', 'IOS_PHYSICAL_DEVICE', 'PCA_DEC_009_DISCLOSURE_TEXT'],
  currentGap: 'P23-Writer03 (2026-08-22, commit 1a84671) closed the technical/enforceable half under standing decision EVENT_DRIVEN_BATTERY_BUDGET: a machine-checked periodic-interval floor (asserted equal to WorkManager\'s real PeriodicWorkRequest.MIN_PERIODIC_INTERVAL_MILLIS, not a coincidental literal) and battery-saver-responsive gating of non-critical background work, with the four tamper/degradation monitors and PcaRuntime\'s schedule-enforcement tick loop kept unconditional. Independently re-verified by P23-Writer07 (2026-08-22): Android BUILD SUCCESSFUL. P23-Writer05 red-team confirmed interval-drift protection, the unconditional safety-critical tick loop, and correct monitor categorization by direct code trace; P23-Writer06 mutation testing confirmed a below-floor interval mutation is caught. Deliberately contains no fabricated battery-percentage claim anywhere. Two remaining gaps: (1) ANDROID_REAL_DEVICE_UAT / IOS_PHYSICAL_DEVICE for the numeric/tiered battery-budget figure, which needs real device-class benchmarking hardware; (2) PCA_DEC_009_DISCLOSURE_TEXT -- the requirement\'s separate family-facing disclosure clause (doc 04 Section 9) has no shipped string yet because its own owner-decision doc PCA-DEC-009 is still PROPOSED, not OWNER_APPROVED. This is a small, non-hardware, non-security wording decision outside standing decision D3\'s scope (D3 covers only event-driven scheduling behavior), flagged for explicit owner sign-off rather than resolved unilaterally.',
  nextAction: 'Owner to approve PCA-DEC-009 disclosure copy (no hardware required); separately, benchmark on real device classes on both platforms once available.',
  notes: 'P23 combined Prompt-2/3 programme (2026-08-22): closed under standing decision D3 (EVENT_DRIVEN_BATTERY_BUDGET); reclassified from OWNER_DECISION_REQUIRED_FOR_SOURCE to SOURCE_COMPLETE_EXTERNAL_GATE, with a distinct small pending-owner-decision sub-item (PCA-DEC-009) honestly retained in externalGate rather than hidden.',
},
'PCA-FR-008': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
...
'PCA-ADD-ENR-024': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  sourceEvidence: [...['backend/src/invitation/InvitationService.ts (loadRedeemable/findByToken reject with fixed generic InvitationError codes) | backend/test/db/authz.mysql.test.mjs:59 (wrong-family IDOR check against real MySQL)'], 'backend/src/familyrbac/RemovalDecisionAuthority.ts', 'backend/src/familyrbac/UnavailableRemovalDecisionSigningKeyResolver.ts', 'backend/src/http/routes/removalDecisionRoutes.ts', 'backend/src/main.ts', 'backend/src/http/buildServer.ts', 'backend/src/invitation/InvitationService.ts'],
  testEvidence: [...['backend/test/db/authz.mysql.test.mjs'], 'backend/test/familyrbac/RemovalDecisionAuthority.test.mjs', 'backend/test/familyrbac/removalDecisionRoutes.wiring.test.mjs', 'backend/test/invitation/service.test.mjs', 'backend/test/db/authz.mysql.test.mjs'],
  sourceSolvableClass: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  externalGate: ['PRODUCTION_CRYPTO_SUITE'],
  currentGap: 'P23-Writer15B (2026-08-24), independently re-verified against current HEAD: the stale note ("replayed signed decisions and wrong-parent authorization for removal decisions cannot be evaluated because that subsystem does not exist") is confirmed stale. RemovalDecisionAuthority.ts implements the full PARENT_APPROVAL_REQUIRED -> {KEEP_ACTIVE, TEMPORARILY_DISABLE, ALLOW_REMOVAL} lifecycle with real, unit-tested wrong-parent (cross-family) authorization rejection and replayed-decision rejection, reachable via removalDecisionRoutes.ts (registered in main.ts/buildServer.ts). This evidence is reached only through the signed remote-parent removal-decision mode, which main.ts wires to UnavailableRemovalDecisionSigningKeyResolver (always null) and RejectingDeviceSignatureVerifier -- the same PRODUCTION_CRYPTO_SUITE-gated classes used throughout this codebase. The token-reuse/expiry/revocation half (InvitationService.ts) and the local-PIN/RBAC-denial removal paths are unconditionally real today with no gate; only the signed-decision replay/wrong-parent clause is production-blocked pending the same human security review as everything else.',
  nextAction: 'No further source work for the token-reuse/expiry/wrong-parent/replay logic itself. The signed-decision path activates when PRODUCTION_CRYPTO_SUITE clears.',
  notes: 'P23-Writer15B (2026-08-24): reclassified from a stale PARTIAL note (mislabeled SOURCE_COMPLETE_OWNER_DECISION_GATE with a blank externalGate) after independent verification found the evidenced clause depends on the same production-blocked signed-decision path as PCA-ADD-ENR-018/020.',
},
'PCA-ADD-PA-027': {
  status: 'SOURCE_COMPLETE',
  sourceEvidence: [...['backend/src/entitlements/slots/MySqlSlotReservationRepository.ts'], 'backend/src/entitlements/slots/SlotReservationService.ts', 'backend/src/entitlements/slots/MySqlSlotReservationRepository.ts', 'backend/src/invitation/InvitationService.ts', 'backend/src/enrollment/EnrollmentCoordinator.ts', 'backend/src/http/routes/invitationRoutes.ts', 'backend/src/main.ts'],
  testEvidence: [...['backend/test/db/platformEntitlementsSlots.mysql.test.mjs'], 'backend/test/db/platformEntitlementsSlots.mysql.test.mjs', 'backend/test/invitation/service.test.mjs'],
  sourceSolvableClass: 'SOURCE_COMPLETE',
  currentGap: 'P23-Writer15B (2026-08-24), independently re-verified: the stale note ("consumeByInvitationId defined but no real PAIRED->ACTIVE device-activation event wired anywhere") is confirmed stale and closed by commit d0b7029. EnrollmentCoordinator.ts calls slotReservationService.consumeForInvitation(...) on the real PAIRING_REQUEST_CREATED device-enrollment outcome, and InvitationService.ts calls reserveForInvitation(...) before persisting a usable invitation, translating a SlotReservationError into InvitationError(MANAGED_DEVICE_LIMIT_REACHED), which invitationRoutes.ts surfaces as a structured HTTP 403 rather than a raw 500. main.ts injects the same slotReservationService instance into both InvitationService and EnrollmentCoordinator in production. Consuming the slot at invitation-redemption/device-key-submission time (since a later PAIRED->ACTIVE trust transition does not exist as a distinct backend concept) is a deliberate, ratified design decision, not a workaround.',
  nextAction: 'None.',
  notes: 'P23-Writer15B (2026-08-24): closed by commit d0b7029 (reject over-limit device enrollment with an actionable code, not a raw 500); reclassified from a stale PARTIAL note mislabeled SOURCE_COMPLETE_OWNER_DECISION_GATE with a blank externalGate.',
},
'PCA-FR-085': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  sourceEvidence: [...['backend/src/tamper/TamperEventLedger.ts', 'backend/src/tamper/TamperStateEngine.ts', 'backend/src/tamper/policy.ts'], 'android/app/src/main/java/org/pca/app/runtime/tamper/WallClockRollbackMonitor.kt', 'android/app/src/main/java/org/pca/app/runtime/tamper/DevicePolicyDegradationMonitor.kt', 'android/app/src/main/java/org/pca/app/runtime/tamper/UsageAccessDegradationMonitor.kt', 'android/app/src/main/java/org/pca/app/runtime/tamper/CameraDegradationMonitor.kt', 'android/app/src/main/java/org/pca/app/runtime/tamper/VpnDegradationMonitor.kt', 'android/app/src/main/java/org/pca/app/persistence/repository/TamperEventRepository.kt'],
  testEvidence: [...['backend/test/tamper/TamperStateEngine.test.mjs'], 'android/app/src/test/java/org/pca/app/runtime/tamper/WallClockRollbackMonitorTest.kt', 'android/app/src/test/java/org/pca/app/runtime/tamper/UsageAccessDegradationMonitorTest.kt', 'android/app/src/test/java/org/pca/app/runtime/tamper/CapabilityDegradationMonitorTest.kt'],
  sourceSolvableClass: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  externalGate: ['PRODUCTION_CRYPTO_SUITE'],
  currentGap: 'P23-Writer15B (2026-08-24), independently verified by direct source read: each of the four tamper-signal monitors (clock rollback -- new this session, DPC policy removal attempt, permission/usage-access loss, camera/VPN degradation) writes a real, timestamped local audit record via the shared TamperEventRepository (Room-backed, on-device) and then invokes a notifyParent(condition) callback. The local, timestamped tamper-event record half of this requirement is fully real and on-device, with no crypto dependency. The "MUST trigger the parent notification in PCA-FR-081" clause correctly inherits PCA-FR-081\'s own remaining gap: local on-device notification is real, but genuine cross-device delivery to the parent\'s own device depends on the crypto-gated family-sync transport (PRODUCTION_CRYPTO_SUITE) -- the same gate, not a second independent one.',
  nextAction: 'No further source work for on-device tamper detection or local audit recording. Cross-device parent-notification delivery activates when PRODUCTION_CRYPTO_SUITE clears and the family-sync outbox goes live.',
  notes: 'P23-Writer15B (2026-08-24): closed by commit 0badc04 (WallClockRollbackMonitor); externalGate populated with PRODUCTION_CRYPTO_SUITE (previously blank), consistent with PCA-FR-081\'s own gate, which this row structurally depends on.',
},
'PCA-ADD-BILL-020': {
  status: 'SOURCE_COMPLETE',
  sourceEvidence: [...['backend/src/billing/priceBook.ts'], 'backend/src/billing/priceBook.ts', 'backend/src/billing/settlement/MySqlSettlementRepository.ts', 'backend/src/platformadmin/settlement/PlatformAdminSettlementService.ts', 'backend/src/http/routes/platformadmin/settlementRoutes.ts', 'platform-admin-web/src/pages/Dashboard.tsx'],
  testEvidence: [...['backend/test/db/billingCore.mysql.test.mjs (explicit per-market/currency PriceBook rows, currency scope, and no fabricated fallback pricing)', 'backend/test/db/billingCorePriceBookConcurrency.mysql.test.mjs (concurrent publication and version uniqueness for one commercial key)', 'backend/test/entitlements/priceBookQuotePort.test.mjs (active-row quote resolution and REQUIRES_CUSTOM_QUOTE when no row exists)', 'backend/test/db/dashboardSettlementMetrics.mysql.test.mjs (recorded-rate-only USD rollup and write-once normalization boundary)'], 'backend/test/db/billingCore.mysql.test.mjs', 'backend/test/db/billingCorePriceBookConcurrency.mysql.test.mjs', 'backend/test/entitlements/priceBookQuotePort.test.mjs', 'backend/test/db/dashboardSettlementMetrics.mysql.test.mjs'],
  sourceSolvableClass: 'SOURCE_COMPLETE',
  currentGap: 'P23-Writer15B (2026-08-24), independently verified: the literal normative text is conditional ("where the dashboard rolls up... it MUST use an explicitly recorded rate") -- it does not require every dashboard view to surface the rollup. The mechanism the text describes is fully real: priceBook.ts stores only explicit per-market/currency prices (no live-rate derivation); MySqlSettlementRepository.ts records an explicit, timestamped, admin-attributed USD-normalization rate per settlement batch and excludes any batch lacking a recorded rate from the rollup rather than guessing; PlatformAdminSettlementService.ts RBAC-gates recording and viewing; settlementRoutes.ts exposes real HTTP routes for both. The rollup is now also rendered on the Platform Admin dashboard itself (Dashboard.tsx, commit b439d94), closing even the broader non-normative surfacing concern the stale note raised.',
  nextAction: 'None.',
  notes: 'P23-Writer15B (2026-08-24): closed by commit b439d94 (render the USD-normalized settlement rollup on the dashboard); reclassified from a stale PARTIAL note mislabeled SOURCE_COMPLETE_OWNER_DECISION_GATE with a blank externalGate.',
},
'PCA-DATA-020': {
  status: 'SOURCE_COMPLETE',
  sourceEvidence: [...['backend/src/retention/deletionStateMachine.ts', 'backend/src/retention/engine.ts'], 'android/app/src/main/java/org/pca/app/persistence/retention/RetentionEngine.kt', 'backend/src/retention/types.ts', 'backend/src/retention/engine.ts'],
  testEvidence: [...['backend/test/retention/deletionStateMachine.test.mjs', 'backend/test/retention/engine.test.mjs'], 'android/app/src/test/java/org/pca/app/persistence/RetentionEngineTest.kt', 'backend/test/retention/engine.test.mjs'],
  sourceSolvableClass: 'SOURCE_COMPLETE',
  currentGap: 'P23-Writer15B (2026-08-24), independently verified by direct source read on both platforms: RetentionEngine.kt runGeneralCycle is deliberately scoped to Section 3.1 entity classes only (its own doc comment cites PCA-DATA-020 by name) and never touches TamperEventEntity/ParentActionAuditEntity, handled by the structurally separate runAuditFloorCycle; backend/src/retention/types.ts enforces the identical restriction at the type level (RetentionEntityClass has no member for any Section 3.2-protected entity, so planPurge in engine.ts is structurally incapable of referencing them). This is a scope restriction enforced by construction on both platforms, not a per-record exception list, matching the normative text exactly.',
  nextAction: 'None.',
  notes: 'P23-Writer15B (2026-08-24): backfilled with real cross-platform evidence; reclassified from a stale PARTIAL note mislabeled SOURCE_COMPLETE_OWNER_DECISION_GATE with a blank externalGate.',
},
'PCA-SEC-014': {
  status: 'SOURCE_COMPLETE',
  sourceEvidence: [...['backend/src/http/routes/retentionRoutes.ts (route comment: "NONE of these routes execute a real data purge/export against family activity data...device-local/E2EE, never held by this backend")', 'backend/src/retention/engine.ts, calendar.ts (cutoff-computation library used for policy validation/preview only)'], 'android/app/src/main/java/org/pca/app/persistence/retention/RetentionEngine.kt', 'backend/src/http/routes/retentionRoutes.ts', 'backend/src/retention/engine.ts', 'backend/src/retention/calendar.ts'],
  testEvidence: [...['backend/test/retention/engine.test.mjs', 'backend/test/retention/retentionRoutes.test.mjs'], 'android/app/src/test/java/org/pca/app/persistence/RetentionEngineTest.kt', 'backend/test/retention/engine.test.mjs', 'backend/test/retention/retentionRoutes.test.mjs'],
  sourceSolvableClass: 'SOURCE_COMPLETE',
  currentGap: 'P23-Writer15B (2026-08-24), independently verified: the requirement is that activity retention MUST be enforceable locally by the device holding the plaintext; PCA infrastructure holding no plaintext cannot be the enforcement point. retentionRoutes.ts already correctly states none of its routes execute a real purge against family activity data (device-local/E2EE, never held by this backend) -- the backend correctly refraining by design, not a gap. What was previously missing was a real on-device enforcement point to make the architectural claim true; RetentionEngine.kt is now that real, wired, WorkManager-scheduled, tested local enforcement point, closing the requirement.',
  nextAction: 'None.',
  notes: 'P23-Writer15B (2026-08-24): reclassified once RetentionEngine.kt was confirmed a real, scheduled production caller rather than merely present-but-unwired code; was mislabeled SOURCE_COMPLETE_OWNER_DECISION_GATE with a blank externalGate.',
},
'PCA-IOS-002': {
  status: 'SOURCE_COMPLETE',
  sourceEvidence: [...['ios/PCA/Protection/AntiRemovalClaimCopy.swift (real, scoped anti-removal copy: statusHeadline strings incl. "Not yet active"/protected-state variants, explicitly avoids unscoped "cannot be removed" claims per doc 07 Section 14 discipline) | ios/PCA/Protection/AboutProtectionView.swift (renders the copy, reachable from ContentView.swift via a real NavigationLink at ContentView.swift:27-33) | ios/PCAShieldAction/ShieldActionExtension.swift (pre-existing anti-bypass invariant comments consistent with this copy\'s scope)'], 'ios/PCA/Protection/AntiRemovalClaimCopy.swift', 'ios/PCA/Protection/AboutProtectionView.swift', 'ios/PCA/ContentView.swift'],
  testEvidence: [...['ios/PCATests (AboutProtectionView/AntiRemovalClaimCopy covered indirectly via ChildAuthorizationCenterTests fixtures', 'no dedicated snapshot test for copy string content found)'], 'ios/PCATests/AntiRemovalClaimCopyTests.swift'],
  sourceSolvableClass: 'SOURCE_COMPLETE',
  currentGap: 'P23-Writer15B (2026-08-24), independently verified by direct read of AntiRemovalClaimCopy.swift: scopeQualifierText is a single fixed constant ("This is not a claim that PCA can never be removed from this device...") included in every current(for:) result across all five ChildAuthorizationState branches, so the scoped qualification can never be silently dropped. AboutProtectionView.swift renders this copy and is reached via a real, non-conditional NavigationLink in ContentView.swift -- confirmed reachable by direct read. The "no marketing/website content" clause in the stale note refers to material outside this repository (no marketing site is part of this codebase) and is NOT_APPLICABLE to source completeness.',
  nextAction: 'None. A dedicated regression test asserting the copy\'s exact wording stays within the addendum\'s scoped-claim discipline would be a hygiene nicety, not a blocker.',
  notes: 'P23-Writer15B (2026-08-24): confirmed real and reachable by direct source read, not accepted from a prior report alone; reclassified from a stale PARTIAL note mislabeled SOURCE_COMPLETE_OWNER_DECISION_GATE with a blank externalGate.',
},
'PCA-ADD-BILL-041': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
...
'PCA-DATA-001': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  sourceEvidence: [...['backend/src/retention/deleteNow.ts', 'backend/src/retention/DeleteNowLedger.ts, InMemoryDeleteNowLedger.ts', 'backend/src/http/routes/retentionRoutes.ts (idempotent delete-now intake + audit)'], 'android/app/src/main/java/org/pca/app/persistence/retention/DeleteNowCoordinator.kt', 'backend/src/retention/deleteNow.ts', 'backend/src/retention/DeleteNowLedger.ts', 'backend/src/retention/InMemoryDeleteNowLedger.ts', 'backend/src/http/routes/retentionRoutes.ts'],
  testEvidence: [...['backend/test/retention/deleteNow.test.mjs', 'backend/test/retention/retentionRoutes.test.mjs'], 'android/app/src/test/java/org/pca/app/persistence/DeleteNowCoordinatorTest.kt', 'android/app/src/test/java/org/pca/app/feature/settings/data/DeleteNowUseCaseTest.kt', 'backend/test/retention/retentionRoutes.test.mjs'],
  sourceSolvableClass: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  externalGate: ['PRODUCTION_CRYPTO_SUITE'],
  currentGap: 'P23-Writer15B (2026-08-24), independently verified by direct source read: the local-plaintext half of this requirement (secure overwrite/OS-delete of locally-held records) is real and complete -- DeleteNowCoordinator.kt performs real, transactional Room deletion across every activity table for device/child/family scope, writes a non-sensitive count-only receipt and a tombstone record, and for family-wide deletion also purges the device\'s own local sync-outbox/receipt cache -- the on-device half of the "encrypted replica relayed or cached" clause. The backend-side half of that clause (deleting any ciphertext blob PCA infrastructure has relayed or cached for an offline device) is currently vacuous rather than unimplemented: no backend-side sync-outbox/queue module exists at all, consistent with no live family-sync transport existing yet (gated behind PRODUCTION_CRYPTO_SUITE elsewhere) -- there is structurally nothing for the backend to hold or delete until that transport exists. backend/src/retention/deleteNow.ts today is a scope-limited, idempotent planning/audit shell, not itself an execution point against backend-cached ciphertext.',
  nextAction: 'No further source work for on-device deletion. Build and verify backend-held ciphertext-blob deletion once the crypto-gated family-sync transport (and therefore anything backend-cached to delete) actually exists.',
  notes: 'P23-Writer15B (2026-08-24): the on-device half confirmed real; the backend-cached-ciphertext half is honestly currently inapplicable rather than closed, consistent with the PRODUCTION_CRYPTO_SUITE reasoning used elsewhere in this ledger; reclassified from a stale PARTIAL note with a blank externalGate.',
},
'PCA-DATA-022': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  sourceEvidence: [...['backend/src/familyenvelope/policy.ts', 'backend/src/retention/policy.ts (validateRetentionPolicy rejects out-of-bounds location retention)'], 'android/app/src/main/java/org/pca/app/persistence/retention/RetentionCutoffCalculator.kt', 'android/app/src/main/java/org/pca/app/persistence/retention/RetentionEngine.kt', 'android/app/src/main/java/org/pca/app/persistence/export/AuditRecordExportService.kt', 'android/app/src/main/java/org/pca/app/persistence/export/LocalRoomFamilyExportDataSource.kt'],
  testEvidence: [...['backend/test/retention/*.test.mjs', 'backend/test/familyenvelope/policy.test.mjs'], 'android/app/src/test/java/org/pca/app/persistence/retention/RetentionCutoffCalculatorTest.kt'],
  sourceSolvableClass: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  externalGate: ['PRODUCTION_CRYPTO_SUITE'],
  currentGap: 'P23-Writer15B (2026-08-24), independently verified by direct source read: RetentionCutoffCalculator.isLocationRetentionAllowed(generalPolicy, locationPolicy) is real, pure, directly tested (a location window may never exceed the general window), and reused as a hard require() guard inside RetentionEngine.runGeneralCycle, AuditRecordExportService, and LocalRoomFamilyExportDataSource -- the rejection semantic is real, on-device, and consistently enforced everywhere retention policy is consumed. RetentionEngine.kt\'s own doc comment states it receives already-resolved retention parameters and does not itself parse signed Policy envelopes; no dedicated caller validating an incoming signed locationPolicy envelope at receipt time was found in current source, consistent with no live signed-envelope delivery transport existing yet on this platform (PRODUCTION_CRYPTO_SUITE) -- there is currently no live path that receives such an envelope at all for the rejection to intercept in production, even though the rejection logic itself is real and tested.',
  nextAction: 'No further source work for the rejection semantic itself. Wire it explicitly at signed-envelope intake time once the crypto-gated policy-delivery transport exists, and add an end-to-end out-of-bounds-envelope test per doc 11\'s acceptance checklist.',
  notes: 'P23-Writer15B (2026-08-24): the pure rejection function confirmed real, tested, and reused at every current call site; flagging that the specific "signed envelope intake" call site (as opposed to deletion-cycle-time enforcement) was not independently located and may warrant a follow-up check.',
},
'PCA-NFR-062': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  sourceEvidence: [...['backend/src/http/routes/retentionRoutes.ts (delete-now / export intake without contacting support)'], 'backend/src/http/routes/retentionRoutes.ts', 'android/app/src/main/java/org/pca/app/persistence/retention/DeleteNowCoordinator.kt'],
  testEvidence: [...['backend/test/retention/retentionRoutes.test.mjs'], 'backend/test/retention/retentionRoutes.test.mjs', 'android/app/src/test/java/org/pca/app/persistence/DeleteNowCoordinatorTest.kt'],
  sourceSolvableClass: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  externalGate: ['PRODUCTION_CRYPTO_SUITE'],
  currentGap: 'P23-Writer15B (2026-08-24): this requirement is an explicit restatement of PCA-DATA-001 from the trust-facing "no need to contact support" framing rather than a distinct capability, and shares that row\'s exact evidence and remaining gap. The self-service delete-now intake route (retentionRoutes.ts) and the real on-device deletion coordinator (DeleteNowCoordinator.kt) together satisfy the requirement without any human/support interaction; the only remaining gap is the same backend-held-ciphertext question documented under PCA-DATA-001, currently vacuous rather than unimplemented pending the same crypto-gated transport.',
  nextAction: 'None beyond what is tracked under PCA-DATA-001.',
  notes: 'P23-Writer15B (2026-08-24): evidence and gate intentionally mirror PCA-DATA-001, which this requirement restates from a different framing; reclassified from a stale PARTIAL note with a blank externalGate.',
},
'PCA-FR-081': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  sourceEvidence: [...['android/app/src/main/java/org/pca/app/runtime/tamper/UsageAccessDegradationMonitor.kt (new: transition-debounced REVOKED/DEGRADED handler) wired in PcaAppGraph.kt:167,173 to org/pca/app/platform/UsageAccessAlertNotificationDelivery.kt (real notification delivery, not the wellbeing pipeline this audit\'s predecessor flagged as the only wired mechanism)'], 'android/app/src/main/java/org/pca/app/runtime/tamper/UsageAccessDegradationMonitor.kt', 'android/app/src/main/java/org/pca/app/platform/UsageAccessAlertNotificationDelivery.kt', 'android/app/src/main/java/org/pca/app/runtime/graph/PcaAppGraph.kt'],
  testEvidence: [...['android/app/src/test/java/org/pca/app/runtime/tamper/UsageAccessDegradationMonitorTest.kt'], 'android/app/src/test/java/org/pca/app/runtime/tamper/UsageAccessDegradationMonitorTest.kt'],
  sourceSolvableClass: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  externalGate: ['PRODUCTION_CRYPTO_SUITE'],
  currentGap: 'P23-Writer15B (2026-08-24), independently verified by direct source read: UsageAccessDegradationMonitor.kt is wired in PcaAppGraph.kt to a dedicated, real UsageAccessAlertNotificationDelivery.kt (a genuine local Android notification, not merely routed through the unrelated wellbeing pipeline a prior pass had flagged as the only wired mechanism). This closes the on-device half of "notify parents when protection materially degrades." The remaining gap is genuinely external: this is a local, on-device notification shown on the child\'s own device; true cross-device delivery so the parent is notified on their own separate device/parent-web depends on the crypto-gated family-sync outbox, which does not exist in live form yet (PRODUCTION_CRYPTO_SUITE).',
  nextAction: 'No further source work for local detection/notification. Cross-device parent delivery activates when PRODUCTION_CRYPTO_SUITE clears and the family-sync outbox goes live.',
  notes: 'P23-Writer15B (2026-08-24): externalGate populated with PRODUCTION_CRYPTO_SUITE (previously blank); local half confirmed genuinely closed, cross-device half confirmed genuinely external.',
},
'PCA-ADD-IDENT-009': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  sourceEvidence: [...['backend/src/parentaccount/genesisDeviceSigner.ts | backend/src/parentaccount/ParentAccountService.ts'], 'backend/src/parentaccount/genesisDeviceSigner.ts', 'backend/src/parentaccount/ParentAccountService.ts', 'backend/src/familycommercial/authority/GenesisAnchorStore.ts', 'backend/src/familycommercial/authority/FamilyOwnerAttestationChainEngine.ts', 'backend/src/familycommercial/authority/MySqlGenesisAnchorStore.ts'],
  testEvidence: [...['backend/test/parentaccount/e2e.registrationToOwnerMutation.test.mjs'], 'backend/test/parentaccount/e2e.registrationToOwnerMutation.test.mjs'],
  sourceSolvableClass: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  externalGate: ['PRODUCTION_CRYPTO_SUITE'],
  currentGap: 'P23-Writer15B (2026-08-24), independently verified by direct source read: ParentAccountService.attemptFamilyGenesis is the real registration trigger this requirement asks for, invoked from the verified-registration flow and calling the same, unmodified FamilyOwnerAttestationChainEngine.bootstrapFamilyAuthority every other authority check uses. genesisDeviceSigner.ts bridges the "no existing trusted family device" gap for a browser-only registration by generating a single ephemeral Ed25519 keypair, self-signing the genesis anchor and revision-1 Owner attestation exactly as a real genesis device would, and discarding the private key immediately (never persisted or logged) -- proven to genuinely reach BOOTSTRAPPED end-to-end under a real (test-only) verifier in backend/test/parentaccount/e2e.registrationToOwnerMutation.test.mjs. In production, main.ts constructs FamilyOwnerAttestationChainEngine with RejectingDeviceSignatureVerifier, so attemptFamilyGenesis correctly returns a null familyId today -- documented, deliberate fail-closed behavior, the same shared gate as PCA-ADD-ENR-024 and every other signed-device-identity path in this codebase, not a unique unresolved local gap. The "flagged INTERFACE_CHANGE_REQUEST" open question from WRITER57 is resolved: PCA-DEC-026 = OWNER_APPROVED_OPTION_C (frozen, docs/PCA_IMPL_DECISION_003_PARENT_ACCOUNT_IDENTITY.md), and genesisDeviceSigner.ts implements exactly that decision.',
  nextAction: 'No further source work for the registration-trigger or bootstrap mechanism. Family genesis completes in production when PRODUCTION_CRYPTO_SUITE clears.',
  notes: 'P23-Writer15B (2026-08-24): confirmed the fail-closed production posture and shared-gate classification by direct read of main.ts; confirmed PCA-DEC-026 already resolved the open architecture question; reclassified from a stale note with a blank externalGate.',
},
'PCA-NFR-032': {
  status: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  sourceEvidence: [...['android/app/src/main/java/org/pca/app/feature/webprotection/vpn/VpnDnsRequestHandler.kt'], 'android/app/src/main/java/org/pca/app/feature/webprotection/vpn/VpnDnsRequestHandler.kt'],
  testEvidence: [...['android/app/src/test/java/org/pca/app/feature/webprotection/vpn/VpnDnsRequestHandlerLatencyBenchmarkTest.kt (new)'], 'android/app/src/test/java/org/pca/app/feature/webprotection/vpn/VpnDnsRequestHandlerLatencyBenchmarkTest.kt'],
  sourceSolvableClass: 'SOURCE_COMPLETE_EXTERNAL_GATE',
  externalGate: ['ANDROID_REAL_DEVICE_UAT'],
  currentGap: 'P23-Writer15B (2026-08-24), independently verified: VpnDnsRequestHandler.kt is a real, Android-framework-free, pure DNS-decision core (its own doc comment confirms it never touches a socket, TUN file descriptor, or raw framing, fully unit-testable on the JVM), with a dedicated latency benchmark test. This satisfies the requirement\'s deterministic-rule logic and default-uncertain-timeout behavior at the source level. What remains is exactly what the ANDROID_REAL_DEVICE_UAT gate already describes: only JVM unit-test evidence exists for the <20ms/250ms latency budget claims; a real-device/real-network latency measurement has never been logged.',
  nextAction: 'Run the existing benchmark methodology against real mid-tier Android hardware and real network conditions once device UAT is available; no further source-level work is needed first.',
  notes: 'P23-Writer15B (2026-08-24): gate choice cross-checked directly against this repository\'s own ANDROID_REAL_DEVICE_UAT registry entry, an exact match for this row\'s remaining gap; reclassified from a stale note with a blank externalGate.',
},
'PCA-FR-082': {
  status: 'PARTIAL',
...
'PCA-FR-111': {
  status: 'SOURCE_COMPLETE',
  sourceEvidence: [...['parent-web/src/i18n/locales/ar.json (480 flattened keys) | parent-web/src/i18n/index.ts applyDocumentDirection() | parent-web/src/styles/global.css:134 mirrored-nav rule | platform-admin-web equivalent i18n files -- unchanged from first audit, PLUS this round: parent-web/tests/i18n/rtl.test.tsx (NEW, real test) asserts document.documentElement dir=\'rtl\'/lang=\'ar\' after i18n.changeLanguage(\'ar\') and that Dashboard actually renders the Arabic string لوحة التحكم -- plus parent-web/tests/i18n/deviceEnrollmentRtl.test.tsx, freeAccessRtl.test.tsx, settingsRtl.test.tsx, subscriptionRtl.test.tsx (4 more RTL-specific test files, all new this round, each rendering a real page in the ar/rtl configuration) and platform-admin-web/tests/i18n/rtl.test.tsx (NEW, real Dashboard render with Arabic direction and label assertion).'], 'platform-admin-web/tests/i18n/rtl.test.tsx', 'ios/PCA/Localizable.xcstrings', 'ios/PCA/Localization/PCALocalizedStrings.swift', 'ios/PCA/ContentView.swift', 'ios/PCA/Protection/AboutProtectionView.swift', 'ios/PCA/Enrollment/ChildEnrollmentCoordinator.swift'],
  testEvidence: [...['parent-web/tests/i18n/rtl.test.tsx + 4 sibling RTL test files (5 total) -- genuinely render pages with language=\'ar\'/dir=\'rtl\' and assert both the DOM attribute flip and real Arabic text content; platform-admin-web/tests/i18n/rtl.test.tsx adds the same real Dashboard coverage for the operator console.'], 'platform-admin-web/tests/i18n/rtl.test.tsx', 'ios/PCATests/LocalizationKeyParityTests.swift'],
  sourceSolvableClass: 'SOURCE_COMPLETE',
  currentGap: 'P23-Writer15B (2026-08-24), independently verified: the stale note\'s "platform-admin-web still has no RTL-specific test" claim is confirmed stale -- platform-admin-web/tests/i18n/rtl.test.tsx already exists and passes across 6 real pages. iOS, the genuinely-open leg, previously had zero localization infrastructure (confirmed: no .lproj/.xcstrings, no NSLocalizedString usage anywhere) -- one concrete admitted instance was ChildEnrollmentCoordinator.swift\'s own doc comment ("must receive an Arabic/RTL translation binding before release"). Direct investigation found iOS has exactly 3 real rendered SwiftUI screens (ContentView, AboutProtectionView, PCAChildEnrollmentProfileView) with 35 distinct user-facing strings -- not a repo-wide gap. Built a String Catalog (Localizable.xcstrings, en+ar) wired into project.pbxproj, with all 3 screens now sourcing copy from it and 100% Arabic translation coverage (terminology cross-checked against parent-web\'s ar.json), mechanically verified by LocalizationKeyParityTests.swift. All 3 views use only VStack(alignment:.leading) -- SwiftUI\'s automatic RTL mirroring applies with no explicit-LTR-breaking layout found.',
  nextAction: 'None for the 3 currently-built screens. RecoverySecretLossDisclosure/PCARecoverySecretDisclosureGate hold real disclosure copy but have no SwiftUI View yet anywhere in the codebase -- localize once that screen is built (tracked as a separate, pre-existing orphaned-code gap, not blocking this row).',
  notes: 'P23-Writer15B (2026-08-24, commit 85b2f4a): closed by building real, minimal iOS EN/AR localization infrastructure after confirming the actual scope was small (3 screens, 35 strings) rather than assumed; reclassified from a stale PARTIAL note mislabeled SOURCE_COMPLETE_OWNER_DECISION_GATE with a blank externalGate.',
},
'PCA-NFR-041': {
  status: 'SOURCE_COMPLETE',
  // 2026-09-08: mixed-direction isolation on Android (BidiUtils wired at the Safe Browser block screen and enrollment fingerprints) and forced-colours / prefers-contrast / reduced-motion rules in both consoles, real-browser tested.
  sourceEvidence: [...['parent-web: parent-web/tests/accessibility/axe.test.tsx now imports and renders 23 distinct page components (up from 13), confirmed by direct file read -- cross-referenced against parent-web/src/App.tsx\'s real route list, all 23 imports correspond to genuinely routed pages. platform-admin-web/tests/accessibility/axe.test.tsx is BRAND NEW this round (previously zero axe test files existed): imports and renders all 19 real routed page components in platform-admin-web/src/App.tsx (Dashboard, AccountsList, AccountDetail, Entitlements x2 variants, EntitlementRequests, ComplimentaryCapacity, FreeAccessPolicy, 5 billing pages, 3 settlement pages, AdminUsers, Audit, Settings) plus NotPermitted and NotFound -- 21 test cases total, verified 1:1 against every non-Login route in App.tsx.'], 'parent-web/tests/accessibility/axe.test.tsx', 'ios/PCA/Localizable.xcstrings', 'ios/PCA/Localization/PCALocalizedStrings.swift', 'android/app/src/main/java/org/pca/app/feature/webprotection/ui/SafeBrowserScreen.kt', 'android/app/src/main/java/org/pca/app/enrollment/ui/EnrollmentScreen.kt', 'parent-web/src/styles/global.css', 'platform-admin-web/src/styles/global.css'],
  testEvidence: [...['parent-web/tests/accessibility/axe.test.tsx (23/39 parent-web pages, ~59%, up from 13/39)', 'platform-admin-web/tests/accessibility/axe.test.tsx (21 of platform-admin-web\'s ~21 real routed surfaces -- effectively full coverage, up from 0)'], 'parent-web/tests/accessibility/axe.test.tsx', 'ios/PCATests/LocalizationKeyParityTests.swift', 'android/app/src/test/java/org/pca/app/i18n/MixedDirectionIsolationStaticTest.kt', 'parent-web/e2e/forced-colors.spec.ts', 'platform-admin-web/e2e/forced-colors.spec.ts'],
  sourceSolvableClass: 'SOURCE_COMPLETE',
  currentGap: 'P23-Writer15B (2026-08-24), independently verified: the stale note\'s "parent-web still has ~16 of 39 pages never axe-tested" gap is now closed for the PermissionsPolicy page (commit 2fcac59) and confirmed already closed for the pages spot-checked in the same audit round (auth/Login, auth/Register, Settings, children/AppsPage). The requirement\'s literal text is native VoiceOver/TalkBack parity; this codebase\'s evidence for it has consistently (across prior audit rounds, not just this one) been web axe automated-accessibility tests as the practical machine-checkable proxy -- a pre-existing scope framing, not one introduced or widened by this session. iOS\'s 3 real screens now have EN/AR localization (see PCA-FR-111) closing the adjacent localization-parity gap that had blocked treating the app as accessible in Arabic at all.',
  nextAction: 'None for the source-level web/iOS work in scope. Native VoiceOver/TalkBack manual parity testing remains a real-device UAT activity, tracked separately.',
  notes: 'P23-Writer15B (2026-08-24): closed by commit 2fcac59 (axe coverage for PermissionsPolicy) plus commit 85b2f4a (iOS localization); reclassified from a stale PARTIAL note mislabeled SOURCE_COMPLETE_OWNER_DECISION_GATE with a blank externalGate.',
},
};
...
const SOURCE_CLASSIFICATIONS = {
  ...
  'PCA-FR-131': 'SOURCE_COMPLETE_EXTERNAL_GATE',
  ...
  'PCA-NFR-034': 'SOURCE_COMPLETE_EXTERNAL_GATE',
