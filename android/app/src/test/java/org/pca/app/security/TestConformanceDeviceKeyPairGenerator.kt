 ...
 * unreviewed [DeviceKeyPairGenerator]. Produces deterministic, correctly-shaped (44-char base64,
 * matching a 32-byte key) public key strings and a fresh, unique alias per call -- never a real
 * cryptographic key pair, never referenced from `PcaAppGraph.kt` or any other production
 * composition file (grep-provable: `PcaAppGraph` only constructs [NotApprovedDeviceKeyPairGenerator]).
 */
class TestConformanceDeviceKeyPairGenerator(
    ...
    private fun generate(alias: String): GeneratedKeyPair {
        ...
        val publicKeyBase64 = Base64.getEncoder().encodeToString(fakeKeyBytes)
