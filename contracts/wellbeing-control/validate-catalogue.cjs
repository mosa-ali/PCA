...
// export-only module, so the CI step `node contracts/wellbeing-control/validate-catalogue.cjs` loaded it and exited 0
...
if (require.main === module) {
  ...
  if (errors.length > 0) {
    console.error(`contracts/wellbeing-control/validate-catalogue.cjs: FAIL (${errors.length} error(s))`);
    for (const error of errors) console.error(` - ${error}`);
    process.exit(1);
  }
  console.log(`contracts/wellbeing-control/validate-catalogue.cjs: OK -- catalogue validated`);
}
