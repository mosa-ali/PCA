...
| `continuousUseLimitMinutes` | positive integer \| `null` | Optional. PCA-SCREEN-BASELINE-1 continuous-use ("60" side) limit, mirroring parent-web `screenTimePolicy.ts`'s field of the same name (15-60 minutes). `null`/absent means this revision carries no continuous-use/break-shield update. Range enforcement is owned entirely by `ScreenTimeBaselinePolicy`/`ScreenTimePolicyApplier` (Android) and `screenTimePolicy.ts` (parent-web) — this document does not itself constrain the value. |
| `breakDurationMinutes` | positive integer \| `null` | Optional. PCA-SCREEN-BASELINE-1 mandatory-break ("30" side) duration, mirroring parent-web `screenTimePolicy.ts`'s field of the same name (30-120 minutes). Same `null`/absent and range-ownership notes as `continuousUseLimitMinutes` above. |
