...
**Status:** v0.2 detailed planning programme — implementation not authorized; staged release model incorporated  
...
9. The programme defines Release A–D readiness, but Claude must not change Azure, DNS, production or store publication without a separate explicit owner authorization. Readiness planning is in scope; deployment mutation is not.
...
## 3. Staged public release model

The programme must evaluate four independent release gates:

### PUBLIC RELEASE A — INFORMATIONAL PUBLIC SITE
`pcasafe.com` may be judged ready before the full PCA application. Required gates: public pages; EN/AR parity; native Arabic sign-off; legally sufficient public/legal pages; accessibility; responsive UAT; security; SEO/performance; public claim audit; privacy wording bounded to proven facts. Release A does **not** require Android, iOS, AI, Parent PWA or advanced protection functionality.

### PUBLIC RELEASE B — AUTH / ACCOUNT ENTRY
Activates Signup, Login, Verify Email, Forgot Password and Reset Password only after real backend/database/email-provider and production-style auth security tests pass. Transactional email acceptance is a dedicated gate under PUBLIC-9.

### PUBLIC RELEASE C — PCA PARENT ENTRY
Activates Parent Web and PWA only after Parent/auth/session/PWA/browser readiness gates pass.

### PUBLIC RELEASE D — PCA CHILD
Activates Android release/distribution only after runtime/emulator acceptance, owner physical-device UAT and distribution/release gates pass. Android remains `COMING_LATER` before then.

A later release being blocked must not block an earlier release that independently satisfies its acceptance criteria. Actual Azure/DNS/store publication remains a separate owner-controlled action after readiness review.
...
### Parallel PPR-2 safety
The Public programme may run in a separate Claude ultracode session while PPR-2 continues, but PUBLIC-0 must identify dirty files, PPR-2-owned paths, Parent Web overlaps, auth overlaps, i18n overlaps and shared backend files. If PPR-2 owns an overlapping file, the Public programme must not edit it concurrently. Public-only files may continue; Auth/PWA/Parent integration must wait or be serialized until ownership is clear.
...
- identify PPR-2 owned/dirty paths and any Parent/auth/i18n/backend overlap;
- produce collision/ownership risk map that explicitly marks concurrent-edit prohibitions.
...
## TESTS
Auth integration tests plus the dedicated **PUBLIC-9A Transactional Email & Account Security Gate**. Required production-style evidence:
...
If the real email provider is not configured or delivery cannot be proven, mark the relevant gate **INFRASTRUCTURE/EXTERNAL BLOCKED**, never PASS.

## BROWSER VALIDATION
Use the real public/auth pages, real backend and authorized safe test accounts. Validate signup → email receipt → verification → login; resend; forgot/reset; expired/replayed links; mobile/desktop; EN/AR; and real-domain links when Release B is being evaluated. Provider delivery logs/screenshots may support evidence but do not replace functional receipt/link tests.

## SECURITY/PRIVACY CHECK
No weakening of password/session/verification controls; no admin auth reuse that merges realms; prevent account enumeration; verify rate limiting; verify one-time/expiry behavior; inspect cookies/CSRF/TLS on the real domain before Release B readiness. Do not broaden this stage into child-activity notification production before those data/privacy paths are ready.

## ARABIC/RTL CHECK
All fields/errors/helper text and password controls.

## ACCEPTANCE CRITERIA
Auth works exactly as securely as baseline or better, with public design integration and bilingual parity, and every applicable PUBLIC-9A transactional email/security check has a real PASS or an explicit blocker status.

## BLOCKERS
Email provider/configuration unavailable, unresolved auth defects, or production-style domain/security prerequisites missing; report as Release B blockers rather than bypassing them.
...
## OBJECTIVE
Assemble an evidence-based **release-by-release** readiness package for owner and primary ChatGPT review without publishing anything. Release A may be ready even when Releases B/C/D remain gated.
...
- classify `PUBLIC_RELEASE_A`, `PUBLIC_RELEASE_B`, `PUBLIC_RELEASE_C` and `PUBLIC_RELEASE_D` separately as READY / NOT_READY / BLOCKED;
- list the exact Azure/DNS/domain/email/store actions that would be required after separate owner authorization without executing them;
- create staged publication-readiness report.
...
## ACCEPTANCE CRITERIA
Package is complete enough for an independent owner/primary ChatGPT decision and each Release A–D has an evidence-based status. Do **not** label production accepted or block Release A solely because B/C/D are not ready.
...
## COMMIT/PUBLICATION RULE
Stop with `READY_FOR_PRIMARY_CHATGPT_REVIEW`. Do not merge to main, change Azure/DNS, publish to stores or release production without a separate owner decision. The report may recommend early Release A when its own gates pass.
...
## 7. Claim-regression testing requirement
...
- Android “Available now”/download actions before Release D evidence;
...
- server-readable family-side synchronized protection payloads or wording that weakens the relay/E2EE invariant;
...
Owner decisions OD-01/02/03/04/05/07/08/09/10/12/14/15 are **APPROVED**; OD-06 is **APPROVED_PROVISIONALLY** subject to legal wording; OD-11 feedback retention and OD-13 legal entity/jurisdiction remain **OWNER_APPROVAL_PENDING**. Unresolved legal/retention matters must not block unrelated Public work, but affected claims/publication remain gated.

## 9. Programme terminal state
