...
### Definition
Where information must move between trusted parent and child endpoints, PCA's approved architecture requires end-to-end encrypted delivery so intermediary central services do not receive readable personal content. **PCA central relay services must not receive readable family-side protection payload content.** This is a critical owner invariant, while the production claim remains `EXTERNAL_SECURITY_REVIEW` until crypto/key/packet/backend evidence is complete.
...
**Critical distinction:** Parent account/authentication and minimum technical service data are necessary operational data. They are **not** equivalent to readable child/family activity content. Family-side synchronization must never become a back door that makes sensitive child/family protection payloads readable to PCA central services.
...
OD-11 remains owner-pending. Revised recommended default: retain identifiable feedback while a case is open, then for up to **90 days after closure**, then delete or de-identify unless a documented legal/security exception applies. This remains subject to owner/legal approval.
