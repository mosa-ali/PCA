...
- `navigateFallback: '/offline.html'` (not `/index.html`) is the SW's only NavigationRoute — plausibly resolving every SW-controlled navigation to the offline page rather than the SPA shell. **Nothing tests it.**
  - *2026-09-08 (post-push verification):* confirmed in a real browser — every worker-controlled reload and deep link returned `offline.html` while online. Fixed: `NetworkOnly` navigation route with a precache fallback; regression `parent-web/e2e/pwa-service-worker.spec.ts`. See `docs/supervision/PCA_FINAL_GAP_ASSESSMENT_2026-09-08.md` §R finding 3.
