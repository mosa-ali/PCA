...
3. **Silent failure / missing feedback on mutating actions**, the same anti-pattern a prior commit already fixed once in `Requests.tsx` (`fix(parent-web): surface Requests-page action failures instead of silently swallowing them`) and which has now been fixed everywhere else it recurred: `WellbeingAdmin.tsx` (`3d43f7d`), `AppsPage.tsx` toggle (**FIXED, `1973cc7`**), `Notifications.tsx` markRead/acknowledge (**FIXED, `bcdcb09`**).

A smaller number of standalone, concrete defects were also found; most are now fixed: `PrayerPage.tsx`'s "Reminders:" string was already using a real i18n key on inspection (stale Stage-0 finding, not a defect); `TrustedBrowser.tsx`'s literal `"(dev)"`/`"(dev stub)"` production button copy is **FIXED (`02a520d`)** — the three genuinely dev-only simulation actions are now demo-mode-gated instead of shown (and silently failing) in production, and the two real actions ("Sign in", "Reset this browser's trust") no longer carry a misleading dev-only label; the page itself was also **unreachable from any navigation** and is now linked from the Security nav section (**FIXED, `63e3be9`**). `Recovery.tsx`'s "Start recovery transaction" button had no `onClick` handler — **FIXED (`914c6d5`, UI-honesty portion)**, see `PCA_P0_DISPOSITION.csv` for the remaining crypto-gate disposition. `ChildLayout.tsx` renders the raw `childId` route param instead of the child's name — **FIXED (`5da2d82`)**, browser-confirmation blocked on `PRODUCTION_CRYPTO_SUITE`. `ProtectionStatus.tsx`'s alert panel hardcoded English text and bypassed i18n entirely — **FIXED (`9c63014`)**. Still open, in platform-admin-web: `AccountsList`/`AccountDetail` readmodel hardcodes every account's status to `'AVAILABLE'`; Platform Admin billing tables (invoices, payments, pricing) render status enums raw/untranslated with no badge (payments/pricing **FIXED, `aeb967c`**; invoices still open); `SettlementBatches`/`SettlementReconciliation` render the reconciliation exception amount as a bare unformatted integer instead of money (**FIXED, `aeb967c`**). `RealDeviceEnrollmentClient` discarding the HTTP 403 body's `code` field is **FIXED (`0763a91`)** — see `PCA_P0_DISPOSITION.csv`.
...
8. `Dashboard.tsx` / `ChildOverview.tsx` (parent) — **FIXED (`37c70a4`)**, `pendingRequestCount` now links to `/requests` when nonzero. `ChildLayout`'s raw-childId heading: **FIXED (`5da2d82`)**, confirmable in-browser only once `PRODUCTION_CRYPTO_SUITE` clears. **Audit correction**: the "runtime-sync API path mismatch" was not a simple rename — `RealParentRuntimeSyncClient`'s parent-browser-facing contract (`/api/sync/status`, `/last-sync`, `/endpoints/:id/{queue,pending}`) has no backend route at all; `backend/src/http/routes/runtimeSyncRoutes.ts`'s `/v1/runtime-sync/*` is a different actor's contract (device-session-authenticated relay for child devices, not parent-browser cookie auth) and cannot be pointed to by simply changing the path. Reclassified `NOT_STARTED` (real backend gap) in `PCA_P0_DISPOSITION.csv`. `importantAlertCount` also has no destination page to link to at all (W02, see `PCA_P0_DISPOSITION.csv` for full detail)
