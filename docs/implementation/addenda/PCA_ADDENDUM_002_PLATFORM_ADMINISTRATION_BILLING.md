...
| Status | AUTHORING COMPLETE; IMPLEMENTATION SUBSTANTIALLY SOURCE-COMPLETE — per `docs/implementation/PCA_COMPLETION_V2_MATRIX.json` at the 2026-09-08 final assessment: 98 SOURCE_COMPLETE (incl. external-gate sub-status), 0 PARTIAL, 0 NOT_STARTED, 0 NOT_APPLICABLE of 98; no row is VALIDATED_COMPLETE (no real-device/provider/UAT evidence exists) and no row is PRODUCTION_READY (every applicable external gate is open). Source lives in `backend/src/{platformadmin,billing,entitlements,familycommercial,commercialmaintenance,commercialnotifications}`, `platform-admin-web/` and twelve-plus billing migrations. (This field read "ARCHITECTURE ONLY — NOT YET IMPLEMENTED" from authoring on 2026-08-13 until 2026-09-08 while the programme was being built; corrected as a status-accuracy fix.) |
...
This addendum specifies that back office as its own, fully-designed architecture: **PCA Platform Administration** (an internal, operator-facing product) and **PCA Billing** (the commercial/payment domain it administers). Both were new source domains at authoring time; both now exist in source (status corrected 2026-09-08 — see the Status row and Section 21). This document is written to the same rigor as the accepted baseline specifically so that, when implementation eventually begins, it does not have to be invented ad hoc by whichever engineer picks it up first.
...
**Status correction 2026-09-08 (final assessment):** implementation of this addendum is substantially source-complete. The table below was authored with every cell `NOT_STARTED` and was never updated while the programme was built; the cells now show the tiers re-derived on 2026-09-08 from executed evidence (backend unit + MySQL suites, platform-admin-web Vitest, Playwright run locally). `VALIDATED_COMPLETE = PARTIAL` means automated evidence exists and was executed but the workstream's acceptance/UAT evidence does not; `PRODUCTION_READY = NO` everywhere because the Section 19 gates are all open. This section extends `docs/architecture/30_IMPLEMENTATION_PROGRAMME.md`'s phase model with the workstreams this addendum requires, using the same three-tier discipline that document's realigned version (Section 22 below, and doc 30 itself) now uses for the accepted-baseline phases.

| Workstream | Scope | SOURCE_COMPLETE | VALIDATED_COMPLETE | PRODUCTION_READY | Notes |
|---|---|---|---|---|---|
| **PCA-PA-1** — Admin Identity/RBAC/Audit | Sections 3, 4, 17: Platform Administration accounts, role model, MFA/step-up, session separation, audit log | SOURCE_COMPLETE | PARTIAL (unit + MySQL incl. cross-realm, TOTP anti-replay, step-up CAS; privilege gate needs `PCA_MIGRATION_DATABASE_URL`) | NO | Foundational — every other PA/BILL workstream depends on this existing first |
| **PCA-PA-2** — Entitlements/Enrollment Limits | Sections 5–8: FREE_STARTER, entitlement model, increase-request flow, concurrency-safe slot reservation | SOURCE_COMPLETE | PARTIAL (unit + MySQL slot-race tests) | NO | Depends on PCA-PA-1; slot-concurrency correctness (`PCA-ADD-PA-038`/039) is release-blocking within this workstream |
| **PCA-PA-3** — Platform Admin Web | Operator-facing UI for Sections 3, 6, 7, 15, 16, 17 | SOURCE_COMPLETE | PARTIAL (153 Vitest; Playwright specs run locally, not in CI) | NO | Depends on PCA-PA-1/2 |
| **PCA-BILL-1** — Billing Core | Section 9 entities (including `PriceBook`/`Quote`/`CommercialMarket`, `PCA-ADD-BILL-002`/`005A`/`045`), Section 10 money model, price-book versioning and quote-snapshot immutability (`PCA-ADD-BILL-043`) | SOURCE_COMPLETE | PARTIAL (unit + MySQL concurrency/immutability) | NO | Depends on PCA-PA-1 (Billing domain is administered through Platform Administration RBAC); explicitly includes the App-Owner-maintained price book (market × currency × target-device-limit × version) and the standard-vs-custom-quote resolution logic (`PCA-ADD-PA-050`) as in-scope deliverables, not a later add-on |
| **PCA-BILL-2** — Payment Provider | Sections 11–13: provider-safe tokenization, provider abstraction, webhook security, and idempotent payment-confirmed entitlement activation (`PCA-ADD-BILL-046`, composed with the concurrency-safe slot model in Section 8.1) | SOURCE_COMPLETE (provider-agnostic; production registry intentionally empty) | PARTIAL (webhook freshness/replay rows annotated `TEST_EVIDENCE_PARTIAL`) | NO | Blocked externally on `PAYMENT_PROVIDER_SELECTION` (Section 19.2) before a concrete adapter can be built, though the abstraction interface (Section 12) can be designed provider-agnostically first; the idempotency-key-on-provider-event-ID design (`PCA-ADD-BILL-046`) is a release-blocking correctness item within this workstream, tested the same way `PCA-ADD-PA-039`'s concurrency-race test is release-blocking for PCA-PA-2 |
| **PCA-BILL-3** — Settlements/Reconciliation | Section 14 | SOURCE_COMPLETE | PARTIAL (unit + MySQL) | NO | Blocked externally on `SETTLEMENT_BANK_CONFIGURATION`/`SUPPORTED_SETTLEMENT_CURRENCIES` for production use; the data model/reconciliation logic can be built and tested against provider sandbox data first |
| **PCA-MYKIDS-BILL-1** — Parent Self-Service | Section 18 additions to Parent Web, including the full ordered self-service flow in Section 18.1 (view allowance → choose/request quantity → see exact price → pay → server-verified confirmation → entitlement increase → notification → immediate UI reflection) | SOURCE_COMPLETE | PARTIAL (Vitest + backend unit/MySQL; no real payment flow can be exercised without a provider) | NO | Depends on PCA-PA-2 and PCA-BILL-1; note `parent-web/src/pages/Subscription.tsx` currently exists only as a static placeholder and is not a starting implementation of this workstream |
| **PCA-PA-4** — Operations/Reporting | Section 15 dashboard, cross-currency reporting (`PCA-ADD-BILL-020`) | PARTIAL (dashboard settlement/service-health metrics and the cross-batch rollup remain open per the traceability R4 notes) | PARTIAL | NO | Depends on PCA-PA-1/2 and PCA-BILL-1 |
| **PCA-PA-5** — Security/Financial Red Team | Threat-model extension of doc 24 covering Sections 4, 11–13, 17 (admin-authority compromise, payment/webhook forgery, settlement tampering) | PARTIAL (security scanner + red-team scripts exist; webhook forgery/replay scenarios only partially test-evidenced; no full financial red team performed) | NO | NO | Required before PCA-BILL-UAT; must include the concurrency-race test (`PCA-ADD-PA-039`) and a webhook-forgery/replay test (`PCA-ADD-BILL-030`–035) as mandatory scenarios, mirroring doc 24 Section 2's mandatory-scenario discipline |
| **PCA-PA-6** — UX/i18n/a11y | Arabic/English + accessibility parity for Platform Admin Web and MyKids Section 18 additions, per doc 20/26 discipline | PARTIAL (exact EN/AR parity; jsdom axe only until the 2026-09-08 real-browser contrast spec; no screen-reader evidence) | PARTIAL | NO | Platform Admin Web is internal/operator-facing; MyKids Section 18 additions MUST meet the same doc 20 bar as the rest of MyKids since it is family-facing |
| **PCA-PA-UAT** | Platform Administration acceptance/UAT | NOT_STARTED | NOT_STARTED | NO | Depends on PCA-PA-1 through PCA-PA-6 |
| **PCA-BILL-UAT** | Billing acceptance/UAT, including a full sandbox-provider end-to-end payment/refund/webhook/settlement drill | NOT_STARTED (no provider selected) | NOT_STARTED | NO | Depends on PCA-BILL-1 through PCA-BILL-3, PCA-MYKIDS-BILL-1, and PCA-PA-5; production go-live additionally requires all Section 19.2 gates closed |
...
As of this addendum's authoring date (2026-08-13) no source existed. As of the 2026-09-08 final assessment the requirement inventory is 98 SOURCE_COMPLETE (incl. external-gate sub-status), 0 PARTIAL, 0 NOT_STARTED, 0 NOT_APPLICABLE of 98; no row is VALIDATED_COMPLETE (no real-device/provider/UAT evidence exists) and no row is PRODUCTION_READY (every applicable external gate is open). Section 21's table carries the per-workstream tiers.
