...
- **PCA-FR-015A** During the mandatory Break Shield, incoming telephone calls remain receivable and answerable, SMS delivery remains functional, and emergency/SOS calling remains unconditionally available. SMS receipt and an ordinary answered call are communication exceptions only: neither ends/resets the break, grants bonus time, nor unlocks unrestricted application use. Active call time is excluded from the required recovery duration; after the call, the Break Shield resumes with the remaining recovery time.
...
- **PCA-FR-043B** PCA SHALL enforce a default daily bedtime protection window from 21:30 local policy time through 07:00 the following day, every day of the week. The cross-midnight schedule continues during offline operation and after process/device restart, is not weakened by daily limits or bonus time, and resumes ordinary policy automatically at exactly 07:00. The lock does not disable telephony, SMS delivery, or emergency functionality.
- **PCA-FR-043C** While PCA-FR-043B is active, incoming calls remain receivable, ring/vibrate according to OS settings, answerable through the OS-native in-call surface, and endable normally. An ordinary call is a temporary communication exception, not a bedtime override; when it ends, bedtime lock resumes unless the 07:00 boundary has passed. Emergency dialing remains permanently available.
