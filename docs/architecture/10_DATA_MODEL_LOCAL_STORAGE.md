...
| Child-profile membership edge (opaque `childProfileId` + `familyId`) | Central service, server-minted at CREATE | No | Identifier only; the readable label is parent-local/E2EE per Section 3.2 | N/A - the row carries no content to encrypt | **Yes / existence and family ownership only** - no readable child content under any name, spelling, encoding, abbreviation or hash (Section 7.1 closed list) | None | Identifier only; never a readable profile | Doc 11 Section 3.2 current family state - never auto-deleted by retention cycling and adds no `RetentionEntityClass` variant; removal is governed by the deferred account-deletion/lifecycle design. Change `CHG-2026-09-04-01` |
