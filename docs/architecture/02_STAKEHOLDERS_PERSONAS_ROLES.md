...
> **SUPERSEDED (commercial-model amendment, not a deletion of history)** — the ">=2 parents / >=2 children with >=1 device each" figure above is the *original* base-family-tier requirement and is kept verbatim for the record. It is now superseded, for the entry/free commercial tier, by the later, controlling architectural decision `PCA-DEC-022` (doc 31) and the FREE_STARTER normative requirements `PCA-ADD-PA-021`..`PCA-ADD-PA-024` (Addendum 002, `docs/implementation/addenda/PCA_ADDENDUM_002_PLATFORM_ADMINISTRATION_BILLING.md` Section 5), which deliberately define the FREE_STARTER entry tier at `parentMemberLimit = 1`, `managedDeviceLimit = 1` — not 2/2. Per `PCA-DEC-022`, this is a narrower on-ramp tier below the paid/standard base tier, not a redefinition of the base tier's structural ambition; a family that needs the >=2/>=2 shape described above self-serves an upgrade (`PCA-ADD-PA-022`) rather than being blocked. Owner decision `PCA-FR-004A = OPTION_B` (2026-08-16) confirms: preserve the FREE_STARTER 1/1 commercial default as-is; do **not** change `parentMemberLimit`/`managedDeviceLimit` to 2/2 to force-match this paragraph's original wording. Implementation status: see `docs/implementation/PCA_COMPLETION_V2_MATRIX.json` (`PCA-FR-004A`) and `docs/implementation/PCA_CURRENT_IMPLEMENTATION_STATUS.md` (FREE_STARTER, SOURCE_COMPLETE, config-table-driven).
