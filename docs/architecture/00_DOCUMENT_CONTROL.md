...
| 2026-09-04 | **Conflict/change entry `CHG-2026-09-04-01` — `CENTRAL_CHILD_MEMBERSHIP_REGISTRY`.** Required by `PCA-DATA-011`, which closes doc 10 Section 7's central-service model with *"this table is the complete list; nothing else is added without a doc 00 Section 9 conflict entry."* **Reason:** doc 08 Section 4's Owner-local *"Create child profile"* step had no durable central record of a child profile's existence, so a `childProfileId` survived only as a side effect of the single enrollment invitation it was minted for, and a family's own child identifiers were unrecoverable across sessions — the Parent → Child → Device enrollment journey could not complete for a new family. **Change:** doc 10 Section 7 gains one central-entity row (*Child-profile membership (opaque)*); doc 10 Section 7.1 states its bounded scope, its closed field list and its non-precedent status; doc 10 Section 7A gains the matching data-flow inventory row; doc 39 gains Section 11 reconciling it with that document's surviving prohibition; doc 18 Section 2 gains the corresponding parent-authority row. **The permission granted is exactly, and only, the owner-approved sentence:** *"The central service may maintain an opaque child-profile membership registry consisting of a server-minted childProfileId bound to familyId. No readable child-profile content is permitted in the central service."* **Affected requirement IDs (no new ID is registered by this entry):** `PCA-DATA-011` — amended, the closed list gains one row; `PCA-DATA-012` — unchanged, the identifier stays opaque and is now server-minted; `PCA-DATA-020` and `PCA-FR-105` — unchanged, the row is doc 11 Section 3.2-protected current family state and gains no `RetentionEntityClass` variant; `PCA-FR-008` — unchanged, age tier stays on the enrollment invitation and on the E2EE `FamilyMember`, never on this registry; `PCA-SEC-023`/`PCA-FR-136` — unchanged, no readable family-activity class is created. **Privacy/security impact — bounded and net positive:** the `(familyId, childProfileId)` fact is already held centrally today by the enrollment-invitation and eye-protection-settings tables; this entry normalises it into one opaque edge and allows the identifier to be server-minted, which removes the client-minted-id cross-family existence oracle. No readable child content is admitted: `displayName`, nickname, `dateOfBirth`, age (including `ageTier`/`ageUxTier` and any age band or birth year), gender, school, avatar/photo, wellbeing, location and usage/activity remain prohibited centrally under any name, spelling, encoding, abbreviation or hash. Doc 09 Section 5.2 is untouched, and doc 39 Section 3's prohibition on a **readable** central child-profile directory **survives intact** — see doc 39 Section 11. **Android/iOS capability impact:** none; no platform capability is claimed, gained or withdrawn on either platform. **Migration impact:** additive only — one new table holding two opaque identifiers, one operational idempotency value and one timestamp; no existing column, table, DTO or contract is altered, and `contracts/child-profile-scope/catalogue.json` needs no change because nothing in it becomes false. **Owner approval state: `APPROVED`**, recorded at `docs/pre-production/PCA_PPR2_OWNER_DECISIONS.md` Part F (`CONTROLLED_DOC_CHANGE = APPROVED, NARROW OPAQUE REGISTRY ONLY`), bounded to the quoted sentence. **This approval is not precedent** — any later proposal to add a readable child field, a lookup returning more than existence/ownership, or a distinguishable "belongs to another family" outcome requires its own Section 9 entry and its own owner approval, and `CHG-2026-09-04-01` may not be cited in support of it. | PPR-2 W4-DOCS |
