...
**PCA-AND-003A** The Android enforcement layer MUST also preserve the telephony infrastructure and SMS transport required for incoming-call delivery and the OS-native incoming/in-call surface required to answer/end a call. Resolution MUST use documented platform roles/capabilities supplied by the device, not an OEM-specific hardcoded dialer package name. `AndroidCommunicationSurfaceResolver` resolves the documented default dialer/SMS surfaces into opaque tokens and `ScheduleRuntime` supplies them to the final evaluator. The resolved communication surfaces are local, non-overridable protected tokens; bedtime, school mode, block periods, daily limits, Break Shield, and parent restrictive scopes cannot intentionally remove them. No Accessibility abuse, hidden system manipulation, or undocumented package-control trick is permitted. This source contract does not by itself prove real-device telephony/SMS behavior.
...
\n## PCA-FR-043B/043C/015A/AND-003A correction closure\n\nThe owner bedtime floor is evaluated as the union of the canonical 21:30-07:00 baseline and all parent-authored BEDTIME windows; explicit parent windows may strengthen but cannot weaken that floor. Communication safety is typed into emergency surfaces, native call surfaces, and SMS transport. Only emergency surfaces short-circuit generic schedule evaluation; answered-call lifecycle state is composed through the public Android telephony observer into PcaRuntime, while SMS delivery is preserved without an unrestricted Messages-app exemption. The observer requires READ_PHONE_STATE, consumes only RINGING/OFFHOOK/IDLE, ignores phone-number data, and unregisters idempotently. If live device authority cannot support schedule enforcement, the runtime reports ENFORCEMENT_UNAVAILABLE rather than claiming a system lock.\n