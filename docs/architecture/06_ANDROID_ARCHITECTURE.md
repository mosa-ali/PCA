...
|---|---|---|
| App usage stats | `VERIFIED_WITH_LIMITATION` (REQUIRES_USER_PERMISSION — Usage Access grant) | `VERIFIED_WITH_LIMITATION` (same API, plus DPC state visibility) |
...
| Local VPN filtering | `VERIFIED_WITH_LIMITATION` (REQUIRES_USER_PERMISSION — one-time VPN consent dialog) | `VERIFIED_WITH_LIMITATION`; stronger persistence via `alwaysOn`/lockdown DPC settings where policy permits |
| Hard non-bypassable break across apps | `UNSUPPORTED` as an absolute claim (best-effort via Accessibility/foreground-service techniques only) | `VERIFIED_WITH_LIMITATION` (REQUIRES_MANAGED_DEVICE — stronger via `setPackagesSuspended`/lock task, still not literally unbypassable against a device-owner-authenticated adult) |
| Suspend selected packages | `UNSUPPORTED` (no ordinary-app authority for `setPackagesSuspended`) | `VERIFIED_WITH_LIMITATION` (REQUIRES_MANAGED_DEVICE — device/profile owner authority) |
| Block uninstall | `UNSUPPORTED` | `VERIFIED_WITH_LIMITATION` (REQUIRES_MANAGED_DEVICE — `setUninstallBlocked` when caller holds device/profile owner authority) |
| Lock-task/kiosk-style break | `UNSUPPORTED` (no general consumer-app guarantee) | `VERIFIED_WITH_LIMITATION` (REQUIRES_MANAGED_DEVICE — allowlisted packages via `setLockTaskPackages`) |
...
Android DPC (Device Policy Controller) APIs can, with proper device-owner or profile-owner authority, suspend packages, set lock-task allowlists, and block uninstall (Section 4). PCA MUST NOT assume every consumer-installed device can enter this authority state — device-owner provisioning generally requires the device to have no pre-existing accounts/apps at provisioning time (a "fresh" or factory-reset state), which is a materially different, higher-friction setup flow than Standard Mode's ordinary app install. This is why Protected Mode provisioning is modeled here as an explicit, separate step in doc 08's enrollment flow, not an automatic upgrade path from Standard Mode.

Provisioning mechanism candidates (see PCA-DEC-002 in doc 01, restated here for the Android-specific decision):
- QR-code-based device-owner provisioning at factory-reset/first-boot time (`android.app.action.PROVISION_MANAGED_DEVICE` via the provisioning QR flow) — matches how existing consumer parental-control and enterprise-adjacent apps provision device-owner outside a formal EMM console. Label: `REQUIRES_FURTHER_OWNER_DECISION` pending confirmation this remains within Android Enterprise's actual distribution terms for a non-EMM consumer app (doc 33 citation, pending verification).
...
- **Standard Mode**: reliable alerts (full-screen notification/overlay at the platform's actual overlay-permission boundary) plus best-supported local restrictions (e.g. returning the user to the PCA Break UI via `Accessibility Service`-assisted app-switch detection where the parent has explicitly granted Accessibility access for this purpose only). Product UI MUST NOT call this "unbreakable" — label `UNSUPPORTED` for any absolute-enforcement claim in Standard Mode; the correct claim is `VERIFIED_WITH_LIMITATION`, and doc 21 documents what happens (parent notification) when a child defeats it.
