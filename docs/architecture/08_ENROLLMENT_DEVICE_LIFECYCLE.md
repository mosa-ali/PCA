...
4. Release platform parental-control authority through supported APIs only — Android: relinquish device/profile owner status via `DevicePolicyManager` supported APIs (doc 06 Section 7) rather than leaving stale DPC state; iOS: remove the Family Controls child-account authorization through Apple's supported flow (doc 07 Section 6) rather than leaving a shield state the family can no longer manage.
...
**PCA-FR-145** Step 4's platform-authority release MUST be attempted and its outcome (succeeded / failed / not-applicable-to-this-mode) recorded in the audit log (PCA-FR-140) — a removal that revokes keys but silently fails to release Android device-owner status or iOS Family Controls authorization would leave the device in a confusing state where PCA cryptographic access is gone but OS-level restriction UI may persist; the parent MUST be told explicitly if step 4 requires a manual follow-up action (e.g. "you may need to manually remove Family Controls restriction via Screen Time settings").
