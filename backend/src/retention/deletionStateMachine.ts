...
const VALID_TRANSITIONS: Record<DeletionState, Partial<Record<DeletionEvent['kind'], DeletionState>>> = {
  ...
  EXPIRY_DUE: { DELETION_REQUESTED: 'DELETE_REQUESTED', LOCAL_DELETE_COMPLETED: 'DELETED_LOCAL' },
  DELETE_REQUESTED: { LOCAL_DELETE_COMPLETED: 'DELETED_LOCAL', REMOTE_UNREACHABLE: 'DELETE_PENDING_REMOTE_DEVICE' },
  DELETED_LOCAL: { REMOTE_ACKNOWLEDGED: 'DELETION_CONFIRMED', REMOTE_UNREACHABLE: 'DELETE_PENDING_REMOTE_DEVICE' },
  DELETE_PENDING_REMOTE_DEVICE: { REMOTE_ACKNOWLEDGED: 'DELETION_CONFIRMED' },
  DELETION_CONFIRMED: {},
};
...
export function applyDeletionEvent(current: DeletionRecordState, event: DeletionEvent): TransitionResult {
  ...
  return { applied: true, state: { ...current, state: nextState } };
}
