...
const REGISTER = new Map([
  ...
  ['MySqlFamilyMembershipRepository', { status: 'GAP', category: 'NO_PRODUCTION_WRITER', note: 'INVESTIGATED METHOD BY METHOD -- this note replaces a class-level assertion with per-method evidence, because the four methods are NOT in one state. (a) applyAcceptedInvitationRoleOnConnection is the ONE method with a real production writer AND reader: VERIFIED IN SOURCE, MySqlFamilyMemberAccountBinder.tryBindAccountToFamilyOnConnection calls it on the acceptance transaction\'s own connection (MySqlFamilyMemberAccountBinder.ts:88), so it is covered by that CERTIFIED row -- parentAccount.mysql.test.mjs asserts the durable family_parent_memberships row it produces (role + status ACTIVE, role taken from the WINNING invitation, and no row created for the losing family). (b) findActiveRole is production-REACHABLE but has NO coverage, and this is the only genuine coverage gap in the class: VERIFIED IN SOURCE, its sole ultimate caller is ParentAccountService.requireFamilyRole (ParentAccountService.ts:545, via this.familyMembershipRepository, which main.ts wires to the explicit durable repository rather than the repository-object fallback at line 109). No test drives ParentAccountService into it. (c) createGenesisAdministrator and (d) applyAcceptedInvitationRole have NO PRODUCTION CALLER, which is what this category names: VERIFIED IN SOURCE, the only references anywhere in src/ are the interface declaration, the implementation, and MySqlParentAccountRepository\'s two forwarding wrappers (lines 437-443) -- nothing calls either wrapper, and their own comment says they exist "for compatibility with existing direct MySQL test composition", i.e. they are a TEST-ONLY surface. THE DUPLICATION THIS EXPOSES is the substantive finding: the real genesis membership write is MySqlGenesisTransactionRepository.ts:110, which has its OWN `INSERT INTO family_parent_memberships`. So createGenesisAdministrator is not merely uncalled, it is a SECOND writer of the same table that production never takes, and the wrapper comment\'s claim of "the single family_parent_memberships persistence implementation; no second role store is created" holds for the CLASS but not for the TABLE. WHAT WOULD CERTIFY THIS ROW: a test driving ParentAccountService.requireFamilyRole against real MySQL so findActiveRole is proven through its real consumer, PLUS a decision on the two uncalled methods (delete them, or repoint the genesis path at createGenesisAdministrator so the duplication is removed) -- the second is a design decision, not a test to write. Deliberately NOT done in this pass: the per-method state was established rather than guessed at, and no promotion was forced on a class whose blocking defect (an uncalled duplicate writer) is a structural choice for the owner.' }],
]);
