...
const REGISTER = new Map([
  ...
  [
    'MySqlParentMfaRepository',
    {
      status: 'CERTIFIED',
      realWriter: 'ParentAccountService.issueCommercialStepUp -> ParentMfaService.issueCommercialStepUp (fresh TOTP verified against the sealed ACTIVE secret written by the real confirmMfaEnrollment path) -> MySqlParentMfaRepository.insertStepUpGrant; the account reaches ACTIVE through the real register -> verify -> first login -> beginMfaEnrollment/confirmMfaEnrollment service calls, never a direct row insert',
      realReader: 'ParentCommercialStepUpAuthority.authorize (the production COMMERCIAL_OWNER_AUTHORITY gate main.ts composes for checkout and every family-commercial mutation) -> ParentMfaService.consumeCommercialStepUp -> MySqlParentMfaRepository.consumeStepUpGrant, and the durable parent_account_security_events audit rows read back from MySQL',
      hostileCase: 'Replay under concurrency: 8 concurrent authorize() calls race one single-use step-up grant and exactly ONE is OWNER_AUTHORIZED, the other 7 are STEP_UP_REQUIRED -- a stolen or re-sent grant can never authorize a second commercial mutation. The same suite races 16 first-family provisions, 12 grace starts, 8 logins with one TOTP code, and 6 confirmations of one enrollment ticket, each winning exactly once.',
      testFile: 'test/db/parentMfa.mysql.test.mjs',
      testName: 'a commercial step-up grant is consumed exactly once under 8 concurrent mutations, and only by the ADMINISTRATOR of that family',
      note: 'ADDED under PCA-DEC-037 (Parent Genesis removed; ADMINISTRATOR + TOTP MFA replaces it). Arrives CERTIFIED as the ratchet requires for a new store. The file is in package.json test:db (CI) and in scripts/run-certified-production-paths.mjs CERTIFIED_FILES. PENDING EMPIRICAL CONFIRMATION: the populated-database zero-skip gate (npm run test:db:certified-paths) had not yet been observed passing for this file when the row was added -- run it before release; if it fails, re-file this row under POPULATED_STATE_FAILURE and raise BASELINE_GAP_COUNT by one rather than weakening the test.',
    },
  ],
...
// 24 -> 19 under PCA-DEC-037: the four Genesis/request-challenge SYNTHETIC_ONLY
// rows and the MySqlFamilyAuthorityGenesisStore CRYPTO_GATED row were REMOVED
// because main.ts no longer constructs those stores at all (Parent Genesis and
