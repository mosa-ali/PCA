...
export class RealSafeZoneClient implements SafeZoneClient {
  ...
  async remove(familyId: string, zoneId: string): Promise<void> {
    ...
    const response = await fetch(this.url(`/api/parent/families/${encodeURIComponent(familyId)}/safe-zones/${encodeURIComponent(zoneId)}`), { method: 'DELETE', credentials: 'include', headers: csrf ? { [CSRF_HEADER_NAME]: csrf } : undefined });
