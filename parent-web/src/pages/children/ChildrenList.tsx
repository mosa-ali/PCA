...
export default function ChildrenList() {
  ...
  const { data, loading, error, errorCause, reload } = useAsync(() => clients.parentFamilyData.getDashboard(), []);

  return (
    <section aria-labelledby="children-title">
      <h1 id="children-title">{t('nav.children')}</h1>
      <AsyncStates loading={loading} error={errorCause ?? error} empty={data !== null && data.children.length === 0} onRetry={reload}>
